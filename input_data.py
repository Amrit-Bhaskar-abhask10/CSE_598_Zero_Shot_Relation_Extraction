...
word2vec_path = data_prefix+'embedding_file.txt'
training_data_path = data_prefix + 'train_shuffle_pos.txt'
test_data_path = data_prefix + 'test_pos.txt'

seen_intent = ['contains', 'nationality', 'place lived', 'place of death', 'administrative divisions', 'neighborhood of', 'country']
unseen_intent = ['place of birth', 'capital']
...
def load_vec(file_path, w2v, class_dict, in_max_len):
    ...
    for line in open(file_path):
        ...
        question = [w.decode('utf8') for w in arr[3].split(' ')]
        pos1 = arr[1]
        pos2 = arr[2]
        positionValues1 = []
        positionValues2 = []
        ...
        for idx in range(len(question)):
            if w2v.vocab.has_key(question[idx]):
                x_arr.append(w2v.vocab[question[idx]].index)
                distance1 = count - int(pos1)
                distance2 = count - int(pos2)

                if distance1 in distanceMapping:
                    positionValues1.append(distanceMapping[distance1])
                elif distance1 <= minDistance:
                    positionValues1.append(distanceMapping['LowerMin'])
                else:
                    positionValues1.append(distanceMapping['GreaterMax'])

                if distance2 in distanceMapping:
                    positionValues2.append(distanceMapping[distance2])
                elif distance2 <= minDistance:
                    positionValues2.append(distanceMapping['LowerMin'])
                else:
                    positionValues2.append(distanceMapping['GreaterMax'])
                count+=1
        ...
        input_pos1.append(np.asarray(positionValues1))
        input_pos2.append(np.asarray(positionValues2))
    ...
    for i in range(len(input_x)):
        if (max_len < s_len[i]):
            ...
            pos1_padding.append(input_pos1[i][0:max_len])
            pos2_padding.append(input_pos2[i][0:max_len])
            continue
        tmp = np.append(input_x[i], np.zeros((max_len - s_len[i],), dtype=np.int64))
        tmp_pos1 = np.append(input_pos1[i], np.zeros((max_len - s_len[i],), dtype=np.int64))
        tmp_pos2 = np.append(input_pos2[i], np.zeros((max_len - s_len[i],), dtype=np.int64))
        x_padding.append(tmp)
        pos1_padding.append(tmp_pos1)
        pos2_padding.append(tmp_pos2)
        
    #pdb.set_trace()
    x_padding = np.asarray(x_padding)
    pos1_padding = np.asarray(pos1_padding)
    pos2_padding = np.asarray(pos2_padding)
    input_y = np.asarray(input_y)
    s_len = np.asarray(s_len)
    return x_padding, pos1_padding, pos2_padding, input_y, s_len, max_len
...
def read_datasets():
    ...
    max_len = 0
    x_tr, x_pos1_tr, x_pos2_tr, y_tr, s_len, max_len = load_vec(
            training_data_path, w2v, sc_dict, max_len)
    x_te, x_pos1_te, x_pos2_te, y_te, u_len, max_len = load_vec(
            test_data_path, w2v, uc_dict, max_len)

    data['x_tr'] = x_tr
    data['x_pos1_tr'] = x_pos1_tr
    data['x_pos2_tr'] = x_pos2_tr
    data['y_tr'] = y_tr
    ...
    data['x_te'] = x_te
    data['x_pos1_te'] = x_pos1_te
    data['x_pos2_te'] = x_pos2_te
    data['y_te'] = y_te
