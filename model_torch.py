...
class CapsuleNetwork(nn.Module):
    ...
    def forward(self, input, input_pos1, input_pos2, len, embedding):
        self.s_len = len
        input = input.transpose(0,1) #(Bach,Length,D) => (L,B,D)
        input_pos1 = input_pos1.transpose(0,1)
        input_pos2 = input_pos2.transpose(0,1)
        # Attention
        if (embedding.nelement() != 0):
            self.word_embedding = nn.Embedding.from_pretrained(embedding)

        emb = self.word_embedding(input)
        emb_e1 = self.pos1_embed(input_pos1)
        emb_e2 = self.pos1_embed(input_pos2)
        
        emb_concat = torch.cat((emb, emb_e1, emb_e2), 2)
        
        #pdb.set_trace()
        packed_emb = pack_padded_sequence(emb_concat,len.clamp(max=100))
    
        #packed_emb = pack_padded_sequence(emb,len.clamp(max=100))
