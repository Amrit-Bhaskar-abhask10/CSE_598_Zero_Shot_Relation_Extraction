...
def setting(data):
    ...
    config['ckpt_dir'] = './saved_models_test/' #check point dir
...
def evaluate_test(data, config, lstm,embedding):
    ...
    x_te = data['x_te']
    x_pos1_te = data['x_pos1_te']
    x_pos2_te = data['x_pos2_te']
    y_te_id = data['y_te']
    u_len = data['u_len']
    y_ind = data['s_label']
    # get unseen and seen categories similarity
    ...
    with torch.no_grad():
        for i in range(test_batch):
            ...
            batch_pos1_original = x_pos1_te[begin_index : end_index]
            batch_pos2_original = x_pos2_te[begin_index : end_index]
            ...
            batch_pos1 = batch_pos1_original[perm_idx]
            batch_pos2 = batch_pos2_original[perm_idx]
            batch_test = batch_test[perm_idx]
            batch_te = torch.from_numpy(batch_te)
            batch_pos1 = torch.from_numpy(batch_pos1)
            batch_pos2 = torch.from_numpy(batch_pos2)

            lstm(batch_te, batch_pos1, batch_pos2, batch_len, embedding)
            ...
            torch_unseen_votes = torch.from_numpy(np.asarray(unseen_votes))
...
def sort_batch(batch_x, batch_pos1, batch_pos2, batch_y, batch_len, batch_ind):
    ...
    batch_pos1_new = batch_pos1[perm_idx]
    batch_pos2_new = batch_pos2[perm_idx]
    batch_y_new = batch_y[perm_idx]
    batch_ind_new = batch_ind[perm_idx]
    #pdb.set_trace()
    return torch.from_numpy(batch_x_new), torch.from_numpy(batch_pos1_new),torch.from_numpy(batch_pos2_new), torch.from_numpy(batch_y_new), \
...
if __name__ == "__main__":
    ...
    x_tr = data['x_tr']
    x_pos1_tr = data['x_pos1_tr']
    x_pos2_tr = data['x_pos2_tr']
    y_tr = data['y_tr']
    y_tr_id = data['y_tr']
    y_te_id = data['y_te']
    y_ind = data['s_label']
    ...
    for epoch in range(config['num_epochs']):
        ...
        for batch in range(batch_num):
            ...
            batch_pos1 = x_pos1_tr[batch_index]
            batch_pos2 = x_pos2_tr[batch_index]
            batch_y_id = y_tr_id[batch_index]
            batch_len = s_len[batch_index]
            batch_ind = y_ind[batch_index]

            # sort by descending order for pack_padded_sequence
            batch_x, batch_pos1, batch_pos2, batch_y_id, batch_len, batch_ind = sort_batch(batch_x, batch_pos1, batch_pos2, batch_y_id, batch_len, batch_ind)

            output = lstm.forward(batch_x, batch_pos1, batch_pos2, batch_len, torch.from_numpy(embedding))
